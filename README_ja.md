...
<img src="images/stereo_2026-03-30_15.43.56.png" alt="SBS screenshot"> 

**SBSShot** は、VR/HMD 向けに **Side-by-Side (SBS) ステレオスクリーンショット**を撮影する Minecraft mod です。

**.** キーで、左目・右目の視点を横並びにした画像ファイルを保存します。

---

## プレビュー

SBS画像ファイルは **SBS Stereo Viewer** で閲覧できます：
...
- `.png`形式で出力
- 保存先：`.minecraft/screenshots/stereo/stereo_YYYY-MM-DD_HH.mm.ss.png`
...
| コマンド | 説明 |
|----------|------|
| `/sbsshot shot` | 現在の視差で SBS ステレオスクリーンショットを撮影 |
| `/sbsshot shot <視差>` | 指定した視差で一度だけ撮影（保存された設定は変更しません） |
| `/sbsshot get_parallax` | 現在の視差を表示 |
| `/sbsshot set_parallax <視差>` | 視差を設定して保存 |

`<視差>` はカメラ間隔（センチメートル）で、範囲は `0.0`〜`30.0` です。
